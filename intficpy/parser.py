...
def extractInline(app, output_string, main_file):
    ...
    while i < len(output_tokens):
        while (
            (i + 1) < len(output_tokens)
            and "<<" in output_tokens[i]
            and not ">>" in output_tokens[i]
        ):
    ...
    for word_ix, word in enumerate(output_tokens):
        if word[0:2] == "<<":
            if not word[-2:] == ">>":
                print(
                    "Bad inline call \033[1m"
                    + word
                    + '\033[0m -- Missing ">>" or missing trailing space'
                )
                continue
            func = word[2:-2]
            if len(func) > 1:
    ...
    if len(nearMatch) == 0:
        ...
        for pair in nearMatch:
            ...
            if len(extra) > 0:
                ...
                removeMatch.append(pair)
            elif (verb.dscope == "direction" and not directionRangeCheck(dobj)) or (
                verb.iscope == "direction" and not directionRangeCheck(iobj)
            ):
                removeMatch.append(pair)
...
def getThing(me, app, noun_adj_arr, scope, far_obj, obj_direction):
    ...
    if len(things) == 0:
        ...
        thing = checkAdjectives(
            app, me, noun_adj_arr, noun, things, scope, far_obj, obj_direction
        )
        return thing
...
def callVerb(me, app, cur_verb, obj_words):
    ...
    if cur_verb.hasIobj:
        ...
        if cur_verb.iscope == "text" or cur_verb.iscope == "direction":
        ...
        ) and roomRangeCheck(me, cur_iobj):
            app.printToGUI(
            ...
            ) and roomRangeCheck(me, cur_dobj):
                app.printToGUI(
                    "(First attempting to take "
                    + cur_dobj.getArticle(True)
                    + cur_dobj.verbose_name
