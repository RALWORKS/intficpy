...
To install, download the intficpy folder, and place inside of a project folder. To use the GUI, you will need to have PyQt5 installed. Refer to examples/terminaltestgame.py for terminal mode. Additionally, the save system depends on pickle. A short introduction to creating games with IntFicPy can be found on the GitHub wiki. Feel free to experiment with the examples. To run an example, place it in intficpy's parent folder, and run it with Python 3.
